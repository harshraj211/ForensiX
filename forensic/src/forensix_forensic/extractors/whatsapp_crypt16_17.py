...
class WhatsAppCrypt16_17Extractor:
    ...
    async def decrypt_crypt16_17(
        ...
        try:
            ...
            if AESGCM is None:
                logger.warning("cryptography library not installed. Simulating decryption for test compatibility.")
                # We raise an error instead of mocking success for real robustness.
                raise DecryptionError("cryptography library is required for actual AES-GCM decryption.")

            try:
                aesgcm = AESGCM(derived_key)
                plaintext = aesgcm.decrypt(actual_iv, ciphertext_with_tag, None)
            except Exception as e:
                raise DecryptionError(f"AES-GCM decryption failed: invalid key, IV, or MAC. {e}")
