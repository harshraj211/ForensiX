    ...
    async def triage_private_space(
        ...
        try:
            ...
            if not user_matches:
                ...
                try:
                    user_output = await self.runner.run_shell_command(
                        serial, "dumpsys user", timeout=15
                    )
                    user_matches = re.findall(
                        r"UserInfo\{(\d+):([^:]+):([0-9a-fA-F]+)\}", user_output
                    )
            ...
            for uid_str, uname, _flags_str in user_matches:
                uid = int(uid_str)
                if uid == 0:
                ...
                try:
                    pkg_output = await self.runner.run_shell_command(
                        serial, f"pm list packages --user {uid}", timeout=15, retries=2
                    )
                    packages = [
                ...
                profiles.append(
                    PrivateSpaceProfileItem(
                        ...
                        user_type="android.os.usertype.profile.PRIVATE"
                        if "private" in uname.lower() or uid >= 10
            ...
            if not profiles:
                raise ArtifactNotFoundError(
                    "No secondary or private space profiles detected on the device."
                )
