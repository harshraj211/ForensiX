...
from .errors import AdbCommandError, DeviceNotConnectedError, AccessDeniedError
        ...
        for attempt in range(retries + 1):
            try:
                ...
                output = await asyncio.wait_for(
                    self.adb.shell(serial, final_command),
                    timeout=timeout
                )
                
                output_str = str(output).strip()
                
                if "device offline" in output_str.lower() or "not found" in output_str.lower():
                    if "device" in output_str.lower():
                        raise DeviceNotConnectedError(f"Device {serial} is disconnected or offline.")
                
                if "permission denied" in output_str.lower() or "inaccessible" in output_str.lower():
                ...
                if attempt == retries:
                    raise AdbCommandError(f"Command '{command}' timed out after {timeout}s.")
                logger.warning(f"ADB command '{command}' timed out. Retrying ({attempt + 1}/{retries})...")
                await asyncio.sleep(1)
            except Exception as e:
                if isinstance(e, (AdbCommandError, DeviceNotConnectedError, AccessDeniedError)):
                    raise
                if attempt == retries:
                    raise AdbCommandError(f"Command '{command}' failed: {e}")
                logger.warning(f"ADB command '{command}' failed: {e}. Retrying ({attempt + 1}/{retries})...")
                await asyncio.sleep(1)
