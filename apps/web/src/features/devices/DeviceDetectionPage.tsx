...
import { ApiError, detectDevices, type DeviceState, type DeviceTransport } from "../../lib/api";
    ...
    <div className="mx-auto max-w-6xl">
      ...
      <div className="mt-7 grid gap-4 lg:grid-cols-[minmax(0,1fr)_300px]">
        <section className="min-w-0" aria-live="polite" aria-busy={detection.isPending}>
          ...
          {detection.data && (
            <DetectionResult devices={detection.data.devices} adbVersion={detection.data.adb.version} />
          )}
        </section>
...
function DetectionResult({ devices, adbVersion }: { devices: DeviceTransport[]; adbVersion: string }) {
  if (devices.length === 0) {
    return (
      <div className="grid min-h-[370px] place-items-center rounded-xl border border-white/8 bg-white/[0.018] px-6 text-center">
        <div className="max-w-md">
  ...
  return (
    <div>
      ...
      <div className="space-y-3">
        {devices.map((device) => (
          <DeviceCard key={`${device.serial}-${device.transport_id ?? "transport"}`} device={device} />
        ))}
      </div>
    </div>
  );
...
function DeviceCard({ device }: { device: DeviceTransport }) {
